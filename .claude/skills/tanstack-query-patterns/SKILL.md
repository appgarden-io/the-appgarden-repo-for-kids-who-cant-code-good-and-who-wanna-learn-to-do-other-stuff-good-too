...
### 4. Use mutate() with Callbacks, Not mutateAsync

For mutations, use `mutate()` with `onSuccess`/`onError` callbacks instead of `mutateAsync` + try/catch:

```typescript
// ✅ Good: mutate() with callbacks
...
// In handler - component-specific side effects go in inline callbacks
...
// ❌ Avoid: mutateAsync with try/catch
...
**Why callbacks over mutateAsync:**
- Hook-level callbacks handle global concerns (cache, toasts)
- Inline callbacks handle component-specific concerns (close modal, clear form)
- Avoids try/catch boilerplate
- Better separation of concerns
- No empty catch blocks needed

**For sequential mutations**, nest the second mutation in the first's `onSuccess`:

```typescript
const handleCreate = () => {
  firstMutation.mutate(data1, {
    onSuccess: (result) => {
      secondMutation.mutate({ ...data2, id: result.id }, {
